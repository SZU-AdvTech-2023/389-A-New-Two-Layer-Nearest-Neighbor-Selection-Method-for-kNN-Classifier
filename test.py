...
for loop_times in range(multirun):   # loop
    print(f"{loop_times + 1}-th:\n")
    result_i = kTLNN(file_path, column, k, kb)
    result = result + result_i
    if result_i < max:
        max = result_i
result = result / multirun
print(f"min error_rate is {max}, average error_rate is {result}")     # print the results in the control window
